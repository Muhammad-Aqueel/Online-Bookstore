...
// Fetch order details
// Only fetch orders that include books from this seller
$orderStmt = $pdo->prepare("
    SELECT o.*, c.code, u.username as buyer_username, u.email as buyer_email, u.phone as buyer_phone
        FROM orders o
        JOIN users u ON o.buyer_id = u.id
        LEFT JOIN coupons c ON c.id = o.coupon_id   -- LEFT JOIN so coupon may be NULL
        WHERE o.id = ?
        AND EXISTS (
            SELECT 1 FROM order_items oi
            JOIN books b ON oi.book_id = b.id
            WHERE oi.order_id = o.id AND b.seller_id = ?)");
...
// Fetch items for this order that are sold by the current seller
...
");
$itemsStmt->execute([$orderId, $user['id']]);
...
if (isset($_POST['update_status'])) {
    ...
    if (!verifyCsrfToken($_POST['csrf_token'] ?? '')) {
        ...
        header('Location: ' . BASE_URL . '/seller/order_details.php?id=' . $orderId);
        exit;
    }

    $newStatus = sanitizeInput($_POST['status']);

    // Validate status value (optional, but good practice)
    ...
    if (!in_array($newStatus, $allowedStatuses)) {
        ...
        try {
            ...
            $_SESSION['success'] = "Order status updated to '" . htmlspecialchars($newStatus) . "' successfully.";
            // Refresh order data
...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="bg-white rounded-lg shadow p-6 mb-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-gray-700">
            <div>
                ...
                <p><strong>Coupon Code:</strong> <?= empty($order['code']) ? 'Not Applied': $order['code'] ?></p>
    ...
    <div class="bg-white rounded-lg shadow p-6">
        <h2 class="text-xl font-semibold text-gray-800 mb-4">Items in This Order (Your Books)</h2>
        <?php if (empty($orderItems)): ?>
            <p class="text-gray-600">No items from your store in this order.</p>
        <?php else: ?>
    ...
    <div class="mt-8 text-center">
        <a href="<?= BASE_URL ?>/seller/orders.php" class="bg-gray-200 text-gray-700 px-6 py-3 rounded-md hover:bg-gray-300 transition duration-300">
            Back to My Orders
        </a>
    </div>
</div>
